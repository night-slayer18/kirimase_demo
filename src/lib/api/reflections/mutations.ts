...
export const updateReflection = async (id: ReflectionId, reflection: UpdateReflectionParams) => {
  ...
  try {
    const [r] =  await db
     ...
     .where(and(eq(reflections.id, reflectionId!), eq(reflections.userId, session?.user.id!)))
     .returning();
    return { reflection: r };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
...
export const deleteReflection = async (id: ReflectionId) => {
  ...
  const { id: reflectionId } = reflectionIdSchema.parse({ id });
  try {
    const [r] =  await db.delete(reflections).where(and(eq(reflections.id, reflectionId!), eq(reflections.userId, session?.user.id!)))
    .returning();
    return { reflection: r };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
